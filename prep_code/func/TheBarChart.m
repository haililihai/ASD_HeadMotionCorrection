...
% Linden Parkes, BMH, 2016
	...
	if length(data) == 1 || length(data) == 2
		...
		if size(data{1},2) == 1
			for i = 1:size(data{1},1)
				% bar(i,data{1}(i),'FaceColor',theColors{},'LineStyle',theLines{i},'LineWidth',2);
				kk=ceil(i/2);
				bar(i,data{1}(i),'FaceColor',theColors{kk},'LineStyle',theLines{kk},'LineWidth',2);
				% bar(2*i-1,data{1}(i),'FaceColor',theColors{i},'LineStyle',theLines{i},'LineWidth',2);
			end
		% elseif size(data{1},2) == 3
		elseif size(data{1},2) > 1
			b1 = bar(data{1});
		...
		if length(data) == 2
			...
			if size(data{2},2) == 1
				for i = 1:numel(data{2})
					...
					% bar(2*i,data{2}(i),'FaceColor',theColors{i},'LineStyle',theLines{i},'LineWidth',2);
					% tempColors = num2cell([255,105,97;97,168,255;178,223,138;117,112,179;255,179,71]./255,2);
					% bar(i,-data{2}(i),'FaceColor',tempColors{5},'LineStyle',theLines{i},'LineWidth',2);
				end
	...
	elseif yLimits(2) <= 40
		...
		% ax.YTick = ([-100:50:100]);
		% ax.YTickLabel = num2str([-[-100:50:-1],[0:50:100]]);
	end
	ax.YLim = (yLimits);
	ax.YDir = Yaxisdir;
	...
	set(hylabel,'VerticalAlignment','top','HorizontalAlignment','right');
	...
	if addText == true
		if length(data) == 1 || length(data) == 2
			if size(data{1},2) == 1
				...
				% text([1:2:7],repmat(yLimits(2) - yLimits(2)*.05,1,size(data{1},1)),num2str(data{1},strprec),... 
				% 'HorizontalAlignment','right',... 
				% 'VerticalAlignment','middle',...
				% 'Color','black',...
				% 'FontSize', FSize,...
		...
		if length(data) == 2
			if size(data{2},2) == 1
				...
				'FontSize', FSize,...
				'Rotation',TextRotation)
				% text([2:2:8],repmat(yLimits(1) - yLimits(1)*.05,1,size(data{2},1)),num2str(data{2},strprec),... 
				% 'HorizontalAlignment','left',... 
				% 'VerticalAlignment','middle',...
				% 'Color','black',...
				% 'FontSize', FSize,...
