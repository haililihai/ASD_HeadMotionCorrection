function [tN,gm,wm,csf,epiBrainMask,t1BrainMask,BrainMask,gmmask,wmmask,csfmask,dvars,dvarsExtract,fdThr,dvarsThr,exclude,outEPI] = prepro_base(cfg)
    ...
    % ------------------------------------------------------------------------------
        ...
        [dirdir,name,ext] = fileparts(cfg.t1name);
        switch ext
            case '.gz'
                cfg.t1name = name;
        end
    ...
    % ------------------------------------------------------------------------------
        ...
        %movefile([cfg.t1dir,cfg.t1name],[cfg.t1dir,cfg.subject])
    ...
    cfg.wm=wm;
    cfg.csf=csf;
    cfg.epiBrainMask=epiBrainMask;
    cfg.t1BrainMask=t1BrainMask;
    cfg.BrainMask=BrainMask;
    cfg.gmmask=gmmask;
    cfg.wmmask=wmmask;
    cfg.csfmask=csfmask;
    cfg.dvars=dvars;
    cfg.dvarsExtract=dvarsExtract;
    cfg.fdThr=fdThr;
    cfg.dvarsThr=dvarsThr;
    cfg.exclude=exclude;
    cfg.outEPI=outEPI;
    save(fullfile(cfg.preprodir,[cfg.WhichProject,'_',cfg.subject,'_',cfg.WhichSessScan,'.mat']), 'cfg');
end
